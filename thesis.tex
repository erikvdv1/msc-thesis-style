...
% Convert .eps images to .pdf (PDFLaTeX)
\ifpdf
\usepackage{epstopdf}
\fi
...
% Make references point to the top of the elements
...
\company{
  \includegraphics[height=2cm]{img/hilbert.eps} \\
  Some Company \\
  With its address \\
  The City, the Netherlands \\
  \httpurl{www.url.nl}
}

% Optional (postscript) cover picture. Put this in comments when not needed.
\coverpicture{\includegraphics[width=13cm]{img/maze.eps}}
...
\newcommand{\note}[1]{\textcolor{grey}{\textbf{\textsc{note:}} #1}}
